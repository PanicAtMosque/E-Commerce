...
// echo convertNumberToWordsForMalaysia(10); die;
...
$record = "INSERT INTO `seller_bill_records` (`bill_id`, `bill_uid`, `bill_name`, `bill_gst`, `bill_tchrg`, `bill_tno`, `bill_amt`, `bill_type`, `bill_entrydt`) VALUES (NULL, '$uid', '$bill_name', NULL, $tr_amt, NULL, '$total', $type, '$entrydt');";
$conn->query($record);
...
for ($i=0; $i < $count; $i++) { 
	...
	$sql = "SELECT pro_id, pro_sell_price, pro_qty FROM product p INNER JOIN category c ON cat_id=pro_grpid INNER JOIN type t ON pro_typeid=ty_id WHERE pro_firmid = $firm AND pro_grpid = $cat AND pro_typeid = $size";
	$pro_id = $conn->query($sql);

	foreach ($pro_id as $key => $value) {
		$pid = $value['pro_id']; //final product id 
		$price = (int)$value['pro_sell_price'];  //product price
		...
		$produts = "INSERT INTO seller_bill_products (bp_uid, bp_pid, bp_qty, bp_price) VALUES ('$uid', $pid, $qty, $price)";
		$conn->query($produts);

	}	
}

$query = "SELECT * FROM `seller_bill_records` WHERE `bill_uid` = '$uid'";
$rec = $conn->query($query);
$pro = "SELECT i.customer_name,c.cat_name,t.ty_name,bp.bp_qty,bp.bp_price,p.pro_price FROM `seller_bill_products` bp INNER JOIN product p ON p.pro_id = bp.bp_pid INNER JOIN category c ON c.cat_id=p.pro_grpid INNER JOIN type t ON t.ty_id=p.pro_typeid INNER JOIN customer_info i ON i.customer_id= p.pro_firmid WHERE bp.bp_uid = '$uid'";
...
<table class="table table-striped table-hover" align="center" border="1px dotted black;" width="75%">
	<tr>
			
	<br>
			<h1><center><strong>shopbysyasya</strong></center></h1>
		<br>
	...
	<tr>
		<td colspan="5"><strong>&nbsp;RM In Words&nbsp;:&nbsp;</strong><?php echo convertNumberToWordsForIndia($value['bill_amt']+$value['bill_tchrg']); ?></td>
